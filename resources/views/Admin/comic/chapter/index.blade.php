    ...
    <div class="flex flex-col">
        ...
        <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
            ...
            <div class="py-2 align-middle inline-block min-w-full sm:px-4 lg:px-8">
                <a href="{{ route('admin.chapter.create', ['id' => $comics->id]) }}"> <button type="button"
                        class="m-2 hover:text-yellow-500 transition ease-out duration-500 profile-btn border-2">Add
                        Chapter</button></a>
                <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead>
                            <tr>
                                <th
                                    class="px m-2 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                                    ID:
                                </th>
                                <th
                        ...
                        <tbody class=" divide-y divide-gray-200">
                            <tr>
                                @foreach ($chapters->sortBy('number') as $chapter)
                                    ...
                                    <td class="px-2 py-4 m-4 whitespace-no-wrap text-right text-sm leading-5 font-medium">
                                        <a href="{{ route('admin.chapter.edit', ['id' => $comics->id, 'chapter' => $chapter->number]) }}"
                                    ...
                                    <td class="px-2 py-4 m-4 whitespace-no-wrap text-right text-sm leading-5 font-medium">
                                        <form action="{{ route('admin.chapter.destroy', [$chapter->id, $comics]) }}"
                                            method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button onclick="return confirm('Are you sure?')"
                                                class="text-red-600 hover:text-indigo-900 btn "

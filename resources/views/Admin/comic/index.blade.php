        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class=" divide-y divide-gray-200">
            <tr> @foreach($comics as $comic)
              ...
              <td class="px py-4 whitespace-no-wrap">
                <div class="text-sm leading-5 ">{{$comic->title}}</div>
              ...
              <td class="px-2 py-4 m-4 whitespace-no-wrap text-right text-sm leading-5 font-medium">
                ...
                <form action="{{ route('admin.comics.destroy', $comic->id)}}" method="post">
                                                      @csrf
                                                         @method('DELETE')
                                                    <button   onclick="return confirm('Are you sure?')" class="text-gray-500 hover:text-yellow-500 btn " type="submit">Delete</button>
                                                  </form>
              </td> 
              <td class="px-2 py-4 m-4 whitespace-no-wrap text-right text-sm leading-5 font-medium">

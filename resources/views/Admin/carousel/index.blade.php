...
@section('content')
    ...
    <div class="flex flex-col">
        <a href="{{ route('admin.carousel.create') }}">
            <h1 class=" font-bold  md:text-xl hover:text-yellow-500 py-4 w-32">Add Carousel</h1>
        </a>
                            ...
                            <tr>
                                @foreach ($carousels as $carousel)
                                    ...
                                    <td class="px-2 py-4 m-4 whitespace-no-wrap text-right text-sm leading-5 font-medium">
                                        <a href="{{ route('admin.carousel.show', $carousel->id) }}"
                                    ...
                                    <td class="px-2 py-4 m-4 whitespace-no-wrap text-right text-sm leading-5 font-medium">
                                        ...
                                        <form action="{{ route('admin.carousel.destroy', $carousel->id) }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button class="text-gray-500 hover:text-yellow-500 btn "
